...
if ($trapper > 0) {
	echo "<script>alert('Email already used!'); window.location='contact.php'</script>";
	echo "<script>javascript:self-history.back() </script>;";
}elseif(strcmp($_SESSION['code_confirmation'], $_POST['code_confirmation']) != 0) {
	echo "<script>alert('The captcha code does not match!!'); window.location='contact.php'</script>";
	echo "<script>javascript:self-history.back() </script>;";
} else {
...
echo "<script>alert('Successfully send your message!'); window.location='contact.php'</script>";
}
}
?>